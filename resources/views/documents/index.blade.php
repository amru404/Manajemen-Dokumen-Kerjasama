            ...
            <el-dropdown class="inline-block">
                ...
                <el-menu anchor="bottom end" popover class="w-56 origin-top-right rounded-md bg-white shadow-lg outline-1 outline-black/5 transition transition-discrete [--anchor-gap:--spacing(2)] data-closed:scale-95 data-closed:transform data-closed:opacity-0 data-enter:duration-100 data-enter:ease-out data-leave:duration-75 data-leave:ease-in dark:bg-gray-800 dark:outline-gray-700">
                    <div class="py-1 space-y-1 px-1" id="statusDropdownItems">
                        ...
                        <button type="button" data-value="denied" class="block w-full text-left px-4 py-2 text-sm font-medium rounded-md bg-error-50 text-error-600 dark:bg-error-500/15 dark:text-error-500 hover:opacity-80">Denied</button>
                        
                        <button type="button" data-value="draft" class="block w-full text-left px-4 py-2 text-sm font-medium rounded-md bg-gray-50 text-gray-600 dark:bg-gray-500/15 dark:text-gray-400 hover:opacity-80">Draft</button>
                        
                        <button type="button" data-value="submitted" class="block w-full text-left px-4 py-2 text-sm font-medium rounded-md bg-blue-50 text-blue-600 dark:bg-blue-500/15 dark:text-blue-400 hover:opacity-80">Submitted</button>
                        
                        <button type="button" data-value="approved" class="block w-full text-left px-4 py-2 text-sm font-medium rounded-md bg-indigo-50 text-indigo-600 dark:bg-indigo-500/15 dark:text-indigo-400 hover:opacity-80">Approved</button>
                        
                        <button type="button" data-value="published" class="block w-full text-left px-4 py-2 text-sm font-medium rounded-md bg-success-50 text-success-600 dark:bg-success-500/15 dark:text-success-500 hover:opacity-80">Published</button>
                        
                        <button type="button" data-value="akan_expired" class="block w-full text-left px-4 py-2 text-sm font-medium rounded-md bg-yellow-50 text-yellow-600 dark:bg-yellow-500/15 dark:text-yellow-400 hover:opacity-80">Akan Expired</button>
                        
                        <button type="button" data-value="expired" class="block w-full text-left px-4 py-2 text-sm font-medium rounded-md bg-gray-100 text-gray-500 dark:bg-gray-600/20 dark:text-gray-400 hover:opacity-80">Expired</button>
                    </div>
                </el-menu>
            </el-dropdown>
        </div>
       ...
       // Status Filter Handler (Untuk custom el-dropdown)
        ...
        dropdownItems.forEach(item => {
            item.addEventListener('click', function() {
                ...
                // 1. Ubah teks tombol utama sesuai status yang dipilih
                if (selectedStatusText) {
                    selectedStatusText.innerText = textLabel;
                }
                
                // 2. Jalankan fungsi filter pencarian pada kolom indeks 8
                table.column(8).search(filterValue).draw();
                
                // 3. Opsional: Menutup popover setelah diklik (jika menggunakan native popover API)
