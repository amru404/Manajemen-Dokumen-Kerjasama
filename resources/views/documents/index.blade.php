...
<script src="https://cdn.jsdelivr.net/npm/@tailwindplus/elements@1" type="module"></script>
        ...
        <div class="overflow-hidden p-5 rounded-2xl border border-gray-200 bg-white pt-4 dark:border-gray-700 dark:bg-white/[0.03]">
          <div id="statusFilterContainer" class="inline-block">
            <el-dropdown class="inline-block">
                <button id="statusDropdownBtn" type="button" class="inline-flex w-full items-center justify-center gap-x-1.5 rounded-md bg-white px-3 py-2 text-sm font-semibold text-gray-900 shadow-xs inset-ring-1 inset-ring-gray-300 hover:bg-gray-50 dark:bg-gray-800 dark:text-gray-200 dark:inset-ring-gray-600 dark:hover:bg-gray-700" aria-expanded="false">
                ...
                <el-menu anchor="bottom end" popover class="w-56 origin-top-right rounded-md bg-white shadow-lg outline-1 outline-black/5 transition transition-discrete [--anchor-gap:--spacing(2)] data-closed:scale-95 data-closed:transform data-closed:opacity-0 data-enter:duration-100 data-enter:ease-out data-leave:duration-75 data-leave:ease-in dark:bg-gray-800 dark:outline-gray-700">
                    <div class="py-1 space-y-1 px-1" id="statusDropdownItems">
                        <button type="button" data-value="" class="block w-full text-left px-4 py-2 text-sm rounded-md text-gray-700 hover:bg-indigo-700 hover:text-white dark:gray-gray-200 dark:hover:bg-gray-700 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-400 dark:hover:bg-indigo-700">Semua Status</button>
                        
                        <button type="button" data-value="denied" class="block w-full text-left px-4 py-2 text-sm rounded-md text-gray-700 dark:text-gray-400 hover:bg-indigo-700 hover:text-white text-gray-900 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-400 dark:hover:bg-indigo-700">Denied</button>
                        
                        <button type="button" data-value="draft" class="block w-full text-left px-4 py-2 text-sm rounded-md text-gray-700 dark:text-gray-400 hover:bg-indigo-700 hover:text-white text-gray-900 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-400 dark:hover:bg-indigo-700">Draft</button>
                        
                        <button type="button" data-value="submitted" class="block w-full text-left px-4 py-2 text-sm rounded-md text-gray-700 dark:text-gray-400 hover:bg-indigo-700 hover:text-white text-gray-900 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-400 dark:hover:bg-indigo-700">Submitted</button>
                        
                        <button type="button" data-value="approved" class="block w-full text-left px-4 py-2 text-sm rounded-md text-gray-700 dark:text-gray-400 hover:bg-indigo-700 hover:text-white text-gray-900 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-400 dark:hover:bg-indigo-700">Approved</button>
                        
                        <button type="button" data-value="published" class="block w-full text-left px-4 py-2 text-sm rounded-md text-gray-700 dark:text-gray-400 hover:bg-indigo-700 hover:text-white text-gray-900 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-400 dark:hover:bg-indigo-700">Published</button>
                        
                        <button type="button" data-value="akan_expired" class="block w-full text-left px-4 py-2 text-sm rounded-md text-gray-700 dark:text-gray-400 hover:bg-indigo-700 hover:text-white text-gray-900 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-400 dark:hover:bg-indigo-700">Akan Expired</button>
                        
                        <button type="button" data-value="expired" class="block w-full text-left px-4 py-2 text-sm rounded-md text-gray-700 dark:text-gray-400 hover:bg-indigo-700 hover:text-white text-gray-900 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-400 dark:hover:bg-indigo-700"">Expired</button>
                    </div>
                </el-menu>
            </el-dropdown>
        </div>
                ...
                <table id="tableDocuments" class="min-w-full whitespace-normal divide-y divide-gray-200 stripe hover w-full text-theme-xs dark:text-gray-400 text-start text-center">
                    <thead class="px-6 py-3.5 border-t border-gray-100 border-y bg-gray-50 dark:border-gray-700 dark:bg-gray-900">
                        <tr>
                            ...
                            <th class="px-6 py-3 break-words text-wrap font-medium text-gray-500 sm:px-6 text-theme-xs dark:text-gray-400 text-start">Pembuat</th>
                            ...
                            <th class="px-6 py-3 break-words text-wrap font-medium text-gray-500 sm:px-6 text-theme-xs dark:text-gray-400 text-start send-email-column @if(auth()->user()->role !== 'admin') hidden @endif">Send Email</th>
                    ...
                    <tbody>
                        @foreach($documents as $d)
                            <tr class="border-b border-gray-100 dark:border-gray-700">
                                ...
                                <td class="px-4 sm:px-6 py-3.5 whitespace-normal break-words">
                                    <p class="text-gray-700 overflow-hidden text-ellipsis text-theme-sm dark:text-gray-400">{{ optional($d->pihak2)->nama ?? '—' }}</p>
                                </td>
                                <td class="px-4 sm:px-6 py-3.5 whitespace-normal break-words">
                                ...
                                <td class="px-4 sm:px-6 py-3.5 whitespace-normal break-words">
                                    <p class="text-gray-700 overflow-hidden text-ellipsis text-theme-sm dark:text-gray-400">{{ optional($d->user)->name ?? '—' }}</p>
                                </td>
                                ...
                                <td class="px-4 sm:px-6 py-3.5 whitespace-normal break-words send-email-column @if(auth()->user()->role !== 'admin') hidden @endif">
                                    @if(($d->user_id == auth()->id() || auth()->user()->role === 'admin') && ($d->status == 'approved' || $d->status == 'published'))
                                ...
                                <td class="px-4 sm:px-6 py-3.5 whitespace-normal break-words">
                                    <div class="flex items-center gap-2">
                                        ...
                                        @if($d->user_id == auth()->id() || auth()->user()->role === 'admin')
    ...
    <script>
        ...
        const table = new DataTable('#tableDocuments', {
            ...
            order: [[0, 'asc']],
            columnDefs: [
                { orderable: false, targets: isAdmin ? [8, 9] : [8] },
